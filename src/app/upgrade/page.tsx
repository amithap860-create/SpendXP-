...
import { FREE_TIER, PREMIUM_TIER, PREMIUM_FEATURES } from '@/config/premium';
...
export default function UpgradePage() {
  ...
  function handleJoinWaitlist(e: React.FormEvent) {
    ...
    setSubmitting(true);
    // Simulate waitlist signup (replace with real API when Stripe is wired)
    setTimeout(() => {
      setSubmitting(false);
      setEmailSent(true);
    }, 800);
  ...
  return (
    <main className="min-h-screen bg-slate-50 pb-24">
      ...
      <div className="bg-[#1A1F2E] text-white">
        <div className="max-w-2xl mx-auto px-4 pt-14 pb-12 text-center">
          ...
          <div className="inline-flex items-center gap-2 bg-amber-500/20 border border-amber-500/30 rounded-full px-4 py-1.5 mb-6">
            <span className="text-amber-400 text-xs font-black uppercase tracking-widest">✨ SpendXP Agent</span>
          </div>
      ...
      <div className="max-w-2xl mx-auto px-4">
        ...
        {/* CTA — Waitlist (swap for Stripe checkout when billing is live) */}
        ...
        <div className="grid grid-cols-3 gap-3 text-center">
          {[
            ...
            { icon: '↩️', label: 'Cancel Anytime', sub: 'No contracts' },
            { icon: '🛡️', label: 'Data Private', sub: 'Never sold' },
          ].map(item => (
