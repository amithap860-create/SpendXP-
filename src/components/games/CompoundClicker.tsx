...
export function CompoundClicker({ onExit }: { onExit: () => void }) {
  ...
  return (
    <div className="max-w-6xl mx-auto grid lg:grid-cols-12 gap-8">
      <div className="lg:col-span-7">
        <Card className={cn("p-12 text-center text-white min-h-[500px] flex flex-col items-center justify-center transition-colors duration-500", activeVehicle.colour)}>
          <div className="text-6xl font-black mb-8">${balance.toLocaleString(undefined, { maximumFractionDigits: 2 })}</div>
          <button 
            onClick={handleClick} 
            className="h-48 w-48 rounded-full bg-white shadow-2xl flex items-center justify-center active:scale-95 transition-transform cursor-pointer min-h-[80px] min-w-[80px]"
          >
            <Coins className={cn("h-24 w-24", activeVehicle.colour.replace('bg-', 'text-'))} />
          </button>
        </Card>
      </div>
        ...
        <Card className="p-4 space-y-3 overflow-auto max-h-[300px]">
          {COMPOUND_VEHICLES.map(v => (
            <div 
              ...
              className={cn("p-4 rounded-xl border-2 cursor-pointer flex items-center gap-4 transition-all min-h-[44px]", activeVehicleId === v.id ? "border-primary bg-primary/5" : "border-slate-100")}
            >
